...
">"==c&&a.match(h))&&(k=1)});else if(a&&a.match(/AND/g)){var g=a.split("AND").length;h.length==g&&b.each(h,function(b,f){var h=RegExp("(\\b|AND)"+f+"(\\b|AND)","g");k="!>"==c&&!a.match(h)||">"==c&&a.match(h)?1:0})}else b.each(h,function(b,f){("!>"==c&&f!=a||">"==c&&f==a)&&(k=1)});k&&f.slideDown("fast")||f.slideUp("fast")}).trigger("change")});"checkbox"==a.data("type")&&d.each(function(c){var f=h.find('[data-id="'+c+'"]').hide();a.find(".zl-field input").bind("change",function(){var a="checked"==
b(this).attr("checked");("!>"==k&&!a||">"==k&&a)&&f.slideDown("fast")||f.slideUp("fast")}).trigger("change")});"radio"==a.data("type")&&d.each(function(c){var f=e,g=h.find('[data-id="'+c+'"]').hide();a.find(".zl-field input").bind("change",function(){var a="checked"==b(this).attr("checked"),c=0,h=b(this).attr("value");a&&(f&&f.match(/OR/g)?(a=RegExp(h,"g"),("!>"==k&&!f.match(a)||">"==k&&f.match(a))&&(c=1)):("!>"==k&&h!=f&&a||">"==k&&h==f&&a&&a)&&(c=1),c&&g.slideDown("fast")||g.slideUp("fast"))}).trigger("change")});
"text"==a.data("type")&&d.each(function(c){var f=h.find('[data-id="'+c+'"]').hide();a.find(".zl-field input").on("keyup change",function(){var a=""!=b(this).val();("!>"==k&&!a||">"==k&&a)&&f.slideDown("fast")||f.slideUp("fast")}).trigger("change")})})}),g.find("[data-dependent]").each(function(){var a=b(this).hide(),c=a.data("dependent").replace(/ /g,""),h=c.match(/AND/g)?c.split("AND"):[],g=c.match(/OR/g)?c.split("OR"):[],q=a.parents(".zlfield.placeholder .wrapper, .zlfield.placeholder").first();
a.on("zlfield-dependent-event",function(b){A_match=0;(A_match=h.length?f.evaluateDependentRules(h,a,q,"AND"):f.evaluateDependentRules(g,a,q,"OR"))&&a.slideDown("fast")||a.slideUp("fast")}).trigger("zlfield-dependent-event")}),g.find("[data-relieson]").each(function(){var a=b(this),c=a.data("relieson"),h=a.parents(".placeholder").find('[data-id="'+c.id+'"]').find("select");h.on("change",function(){var g=b(this).val()?b(this).val():"",q=b(this).closest(".zl-field"),k=c.json,m=a.attr("data-control"),
n=b(this).closest(".zlfield-main").attr("data-ajaxargs"),l=c.psv,p=c.id;l[c.id]=g;q.append(b('<span class="activity zl-loader">'));b.ajax({type:"POST",url:c.url+"&task=loadfield",data:{json:k,ctrl:m,psv:l,pid:p,node:null,args:n,ajaxcall:!0,enviroment:f.options.enviroment,enviroment_args:f.options.enviroment_args}}).done(function(c){c=b.parseJSON(c);q.find(".activity").remove();a.slideUp("fast",function(){a.empty();c&&c.result&&a.html('<div class="loaded-fields">'+c.result+"</div>");f.actions(a.find("> .loaded-fields"));
...
c.fadeOut("fast",function(){b(this).html(a.result);f.actions(c);c.fadeIn("slow")})}})}),b("[data-type=items] .zl-field",g).each(function(){var a=b(this),c=b("input.zlux-x-dummy",a),f=c.val(),g=c.data("params");c.remove();b.zlux.assets.load("fields").done(function(){a.zlux("fields",b.extend({},g,{field:"items",controlName:f}))})}),b("[data-type=options] .zl-field",g).each(function(){var a=b(this),c=b("input.zlux-x-dummy",a),f=c.val(),g=c.data("params");c.remove();b.zlux.assets.load("fields").done(function(){a.zlux("fields",
b.extend({},g,{field:"options",controlName:f}))})}),g.data("zlfield-actions-init",!0))},evaluateDependentRules:function(g,f,a,c){var h=0,l=1;g.each(function(g){if(!l)return!1;var k=g.split("!="),m=2==k.length?"!=":"==",k="=="==m?g.split("=="):k;field=a.find('[data-id="'+k[0]+'"]');type=field.data("type");unique_id=c+"-"+f.data("id")+"-"+field.data("id")+"-"+a.data("id");e=k[1].replace("NONE","");if("select"==type||"itemLayoutList"==type||"layout"==type||"apps"==type||"types"==type||"elements"==type||
"modulelist"==type||"separatedby"==type){g=field.find(".zl-field select");var k=b.makeArray(g.val()),n=0;if(e&&e.match(/OR/g))b.each(k,function(a,b){var c=RegExp("(\\b|OR)"+b+"(\\b|OR)","g");("!="==m&&!e.match(c)||"=="==m&&e.match(c))&&(n=1)});else if(e&&e.match(/AND/g)){var r=e.split("AND").length;k.length==r&&b.each(k,function(a,b){var c=RegExp("(\\b|AND)"+b+"(\\b|AND)","g");n="!="==m&&!e.match(c)||"=="==m&&e.match(c)?1:0})}else b.each(k,function(a,b){("!="==m&&b!=e||"=="==m&&b==e)&&(n=1)});h=n;
g.data("zlfield-dependent-"+unique_id+"-init")||(g.on("change",function(){f.trigger("zlfield-dependent-event")}),g.data("zlfield-dependent-"+unique_id+"-init",!0))}"checkbox"==type&&(g=field.find(".zl-field input"),n=0,k="checked"==g.attr("checked"),("!="==m&&!k||"=="==m&&k)&&(n=1),h=n,g.data("zlfield-dependent-"+unique_id+"-init")||(g.on("change",function(){f.trigger("zlfield-dependent-event")}),g.data("zlfield-dependent-"+unique_id+"-init",!0)));if("radio"==type){g=field.find(".zl-field input");
var n=0,p=e;g.each(function(){var a="checked"==b(this).attr("checked"),c=b(this).attr("value");a&&(p&&p.match(/OR/g)?(a=RegExp(c,"g"),("!="==m&&!p.match(a)||"=="==m&&p.match(a))&&(n=1)):("!="==m&&c!=p&&a||"=="==m&&c==p&&a&&a)&&(n=1))});h=n;g.data("zlfield-dependent-"+unique_id+"-init")||(g.on("change",function(){f.trigger("zlfield-dependent-event")}),g.data("zlfield-dependent-"+unique_id+"-init",!0))}"text"==type&&(g=field.find(".zl-field input"),k=""!=g.val(),("!="==m&&!k||"=="==m&&k)&&(n=1),h=n,
g.data("zlfield-dependent-"+unique_id+"-init")||(g.on("keyup change",function(){f.trigger("zlfield-dependent-event")}),g.data("zlfield-dependent-"+unique_id+"-init",!0)));if("AND"==c&&!h||"OR"==c&&h)l=!1});return h}});b.fn[l.prototype.name]=function(){var g=arguments,f=g[0]?g[0]:null;return this.each(function(){var a=b(this);if(l.prototype[f]&&a.data(l.prototype.name)&&"initialize"!=f)a.data(l.prototype.name)[f].apply(a.data(l.prototype.name),Array.prototype.slice.call(g,1));else if(!f||b.isPlainObject(f)){var c=
new l;l.prototype.initialize&&c.initialize.apply(c,b.merge([a],g));a.data(l.prototype.name,c)}else b.error("Method "+f+" does not exist on jQuery."+l.name)})}})(jQuery);