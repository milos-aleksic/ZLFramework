...
a.zluxdialog.dialog("close")});a.initMainToolbar();a.zluxdialog.newSubToolbar("filter","main");a.zluxdialog.newSubToolbar("newfolder","main");a.initUploader()},initMainEvents:function(){var a=this;a.bind("InitComplete",function(b){a.zluxdialog.scrollbar("refresh");b=c(".zlux-dialog-subtoolbar-filter",a.zluxdialog.toolbar.wrapper);c(".zlux-x-filter-input_wrapper",a.oTable.fnSettings().nTableWrapper).appendTo(b);a.zluxdialog.initContent()}).bind("BeforeDeleteFile",function(b,d){if(!c(".zlux-x-details-message-actions")[0]){var e=
c("<div>"+a._("folder"==d.type?"DELETE_THIS_FOLDER":"DELETE_THIS_FILE")+'<span class="label label-warning label-link">'+a._("CONFIRM").toLowerCase()+"</span></div>").on("click",".label-link",function(){c(this).data("submited")||(c(this).data("submited",!0),c(".column-icon i",d.dom).addClass("icon-spinner icon-spin"),a.deleteObject(d).done(function(){d.dom.fadeOut("slow",function(){c(this).remove();var b=a.oTable.fnSettings().aAjaxDataCache[a.oTable.fnSettings().sCurrentPath].aaData;c.each(b,function(a,
c){if(d.dom.data("id")==c.name&&d.dom.data("type")==c.type)return b.splice(a,1),!1})})}).fail(function(b){a.pushMessageToObject(d,b)}).always(function(a){c(".column-icon i",d.dom).removeClass("icon-spinner icon-spin")}))});a.pushMessageToObject(d,e)}})},initMainToolbar:function(){var a=this;a.zluxdialog.setMainToolbar([{title:a._("APPLY_FILTERS"),icon:"filter",click:function(b){a.zluxdialog.toggleSubtoolbar("filter","main");b.parent().siblings().children("i").not(b).removeClass("zlux-ui-tool-enabled");
