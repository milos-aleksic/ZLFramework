...
b("table",a).dataTable({sDom:"F<'row-fluid'<'span12'B>><'row-fluid'<'span12't>>",oLanguage:{sEmptyTable:"No files found",sInfoEmpty:""},sAjaxUrl:c.AjaxURL(),sAjaxSource:d,sServerMethod:"POST",sStartRoot:c.cleanPath(c.options.root),bPaginate:!1,aoColumns:[{sTitle:"",mData:"type",bSearchable:!1,sWidth:"14px",sClass:"column-icon",mRender:function(a,c,b){return"display"==c?'<i class="icon-'+("folder"==a?"folder-close":"file-alt")+'"></i>':a}},{sTitle:"Name",mData:"name",sClass:"column-name",mRender:function(a,
c,b){return"display"==c?"":a},fnCreatedCell:function(a,d,g,k,f){b(a).parent("tr").attr("data-id",c.cleanPath(g.name))}}],aoColumnDefs:{bVisible:!1,aTargets:[2]},aaSorting:[[0,"desc"],[1,"asc"]],fnServerData:function(a,b,d,k){c._fnServerData(a,b,d,k)},fnServerParams:function(a){a.push({name:"extensions",value:c.options.extensions});"s3"==c.options.storage&&(a.push({name:"storage",value:"s3"}),a.push({name:"accesskey",value:c.options.storage_params.accesskey}),a.push({name:"key",value:c.options.storage_params.secretkey}),
a.push({name:"bucket",value:c.options.storage_params.bucket}))},fnRowCallback:function(a,d,g,k){d.dom=b(a);d.dom.attr("data-type",d.type).addClass("zlux-object");b(".column-name",d.dom).html("").removeClass("zlux-ui-open").append(c.renderObjectDOM(d));b(".zlux-x-name",d.dom).append('<i class="zlux-x-name-edit icon-edit-sign" />')},fnInitComplete:function(d,h){var g=b(".zlux-x-filter-input_wrapper",a).append(b('<i class="icon-search" />'),b('<i class="icon-remove zlux-ui-dropdown-unselect" />').hide().on("click",
function(){b("input",g).val("");b(this).hide();c.oTable.fnFilter("")}));b("input",g).on("keyup",function(){""==b(this).val()?b(".zlux-ui-dropdown-unselect",g).hide():b(".zlux-ui-dropdown-unselect",g).show()});c.trigger("InitComplete")},fnPreDrawCallback:function(a){c.zluxdialog.spinner("show")},fnDrawCallback:function(a){var d=a.oInstance.fnPagingInfo();a=b(".dataTables_paginate",b(a.nTableWrapper)).closest(".row-fluid");1>=d.iTotalPages&&a.hide()||a.show();c.zluxdialog.scrollbar("refresh");c.zluxdialog.spinner("hide")}}).on("click",
".zlux-object .zlux-x-name a",function(){var a=b(this).closest("tr.zlux-object"),d=c.oTable.fnGetData(a[0]),g=c.oTable.fnSettings();d.dom=a;"true"!=d.dom.attr("data-zlux-object-status")&&(d.dom.attr("data-zlux-object-status","true"),d.dom.siblings().removeAttr("data-zlux-object-status"),"folder"==d.dom.data("type")&&(c.oTable.fnSettings(),g.sGoToPath=d.dom.data("id"),c.oTable.fnReloadAjax(g.sAjaxSource)),c.trigger("ObjectSelected",d));return!1}).on("click",".zlux-object .zlux-x-remove",function(){var a=
b(this).closest("tr.zlux-object"),d=b("td",a),g=c.oTable.fnGetData(a[0]);g.dom=a;d.hasClass("zlux-ui-open")?c.trigger("BeforeDeleteFile",g):row.removeAttr("data-checked");return!1})},_fnServerData:function(a,c,d,e){var h=this,g;e.aAjaxDataCache=e.aAjaxDataCache?e.aAjaxDataCache:{};b.isEmptyObject(e.aAjaxDataCache)&&(e.sCurrentPath=e.oInit.sStartRoot);g=h.cleanPath(e.sCurrentPath+"/"+e.sGoToPath);e.sGoToPath="";c.push({name:"root",value:g});e.jqXHR=b.ajax({url:a,data:c,beforeSend:function(a,c){var f=
!1;if(!e.bReloading){var l=e.aAjaxDataCache[g];l&&(e.sCurrentPath=g,b(e.oInstance).trigger("xhr",[e,l]),d(l),f=!0)}if(f)return!1;h.zluxdialog.spinner("show")},success:function(a){a.sError&&e.oApi._fnLog(e,0,a.sError);b.isEmptyObject(e.aAjaxDataCache)&&(e.oInit.sStartRoot=a.root);e.sCurrentPath=a.root;e.bReloading&&(e.aAjaxDataCache={});e.aAjaxDataCache[a.root]=a;e.bReloading=!1;b(e.oInstance).trigger("xhr",[e,a]);d(a)},dataType:"json",cache:!1,type:e.sServerMethod,error:function(a,c,b){"parsererror"==
c&&e.oApi._fnLog(e,0,"DataTables warning: JSON data from server could not be parsed. This is caused by a JSON formatting error.")}})},_getFullPath:function(a){var c=this.oTable.fnSettings().sCurrentPath;return c?c+"/"+a:a},cleanPath:function(a){return a.replace(/undefined/g,"").replace(/\/\//g,"/").replace(/(^\/|\/$)/g,"")},_getRowFromPath:function(a){return b('tr[data-path="'+a+'"]',this.oTable)},renderObjectDOM:function(a){var c="";b.each("folder"==a.type?[{name:"Name",value:a.basename}]:[{name:"Name",
...
this,d=[];a=c._getFullPath(a);d=c.pushStorageData(d);"s3"==c.options.storage&&(a+="/");d.push({name:"path",value:a});return b.Deferred(function(a){b.ajax({url:c.AjaxURL()+"&task=newFolder",data:d,dataType:"json",type:"post"}).done(function(b){b.result?a.resolve(b.name):a.reject(b.errors)}).fail(function(){a.reject("Something went wrong, the task was not performed.")})}).promise()},renameObject:function(a){var c=this,d=b(".zlux-x-name a",a.dom),e=d.html().match(/\.+[a-zA-Z]+$/g),h=d.html().replace(/\.+[a-zA-Z]+$/g,
""),e=null!==e?e:"",g=b('<div>Input the new name <br /><input class="zlux-x-input" type="text" value="'+h+'" /> '+e+'<br />and <span class="label label-success label-link">confirm</span></div>').on("click",".label-link",function(){b(this).data("submited")||(b(this).data("submited",!0),b(".column-icon i",a.dom).addClass("icon-spinner icon-spin"),c._changeObjectName(a,b("input",g).val()+e).done(function(c){d.html(c);b(".zlux-x-details-content ul li:first span",a.dom).html(c);a.dom.attr("data-id",c);
a.name=c;a.basename=c.replace(/\.+[a-zA-Z]+$/g,"");a.path=a.path.replace(/(\w|[-.])+$/,c);b(".zlux-x-msg",a.dom).remove()}).fail(function(b){c.pushMessageToObject(a,b)}).always(function(c){b(".column-icon i",a.dom).removeClass("icon-spinner icon-spin")}))}).on("keypress","input",function(a){13==(a.keyCode?a.keyCode:a.which)&&b(".label-link",g).trigger("click")});c.pushMessageToObject(a,g)},_changeObjectName:function(a,c){var d=this,e=[],h=d._getFullPath(a.name);dest=d._getFullPath(c);e=d.pushStorageData(e);
"s3"==d.options.storage&&"folder"==a.type&&(h+="/",dest+="/");e.push({name:"src",value:h});e.push({name:"dest",value:dest});return b.Deferred(function(a){b.ajax({url:d.AjaxURL()+"&task=moveObject",data:e,dataType:"json",type:"post"}).done(function(b){b.result?a.resolve(b.name):a.reject(b.errors)}).fail(function(){a.reject("Something went wrong, the task was not performed.")})}).promise()}});b.fn[f.prototype.name]=function(){var a=arguments,c=a[0]?a[0]:null;return this.each(function(){var d=b(this);
...
this;a.bind("InitComplete",function(c){a.zluxdialog.scrollbar("refresh");c=b(".zlux-dialog-subtoolbar-filter",a.zluxdialog.toolbar.wrapper);b(".zlux-x-filter-input_wrapper",a.oTable.fnSettings().nTableWrapper).appendTo(c);a.zluxdialog.initContent()}).bind("BeforeDeleteFile",function(c,d){if(!b(".zlux-x-details-message-actions")[0]){var e=b('<div>You are about to delete this file, please <span class="label label-warning label-link">confirm</span></div>').on("click",".label-link",function(){b(this).data("submited")||
(b(this).data("submited",!0),b(".column-icon i",d.dom).addClass("icon-spinner icon-spin"),a.deleteObjectFile(d).done(function(){d.dom.fadeOut("slow",function(){b(this).remove();var c=a.oTable.fnSettings().aAjaxDataCache[a.oTable.fnSettings().sCurrentPath].aaData;b.each(c,function(a,b){if(d.dom.data("id")==b.name&&d.dom.data("type")==b.type)return c.splice(a,1),!1})})}).fail(function(b){a.pushMessageToObject(d,b)}).always(function(a){b(".column-icon i",d.dom).removeClass("icon-spinner icon-spin")}))});
a.pushMessageToObject(d,e)}})},initMainToolbar:function(){var a=this;a.zluxdialog.setMainToolbar([{title:"Apply Filters",icon:"filter",click:function(b){a.zluxdialog.toggleSubtoolbar("filter","main");b.parent().siblings().children("i").not(b).removeClass("zlux-ui-tool-enabled");b.toggleClass("zlux-ui-tool-enabled")}},{title:"New folder",icon:"folder-close",subicon:"plus-sign",click:function(c){a.zluxdialog.toggleSubtoolbar("newfolder","main");b(".zlux-dialog-subtoolbar-newfolder",a.zluxdialog.toolbar.wrapper).html("").append(b('<input type="text" class="zlux-x-input" placeholder="Folder name" />').on("keypress",
function(c){13==(c.keyCode?c.keyCode:c.which)&&(a.createFolder(b(this).val()),a.zluxdialog.spinner("show"),a.createFolder(b(this).val()).always(function(b){a.reload()}))}));c.parent().siblings().children("i").not(c).removeClass("zlux-ui-tool-enabled");c.toggleClass("zlux-ui-tool-enabled")}},{title:"Upload files to current folder",icon:"cloud-upload",click:function(){a.zluxdialog.showToolbar(2);a.zluxdialog.scrollbar("hide");b(".zlux-filesmanager",a.zluxdialog.content).fadeOut("400",function(){a.zluxupload.inited||
a.zluxupload.init();a.zluxupload.options.path=a.oTable.fnSettings().sCurrentPath;b(".zlux-upload",a.zluxdialog.content).fadeIn("400")})}},{title:"Refresh",icon:"refresh",click:function(){a.reload()}}])},initUploader:function(){var a=this;a.zluxupload=new b.fn.zluxUpload({path:"images",wrapper:a.zluxdialog.content,storage:a.options.storage,storage_params:a.options.storage_params,max_file_size:a.options.max_file_size,extensions:a.options.extensions});a.zluxupload.bind("QueueChanged",function(b){a.zluxdialog.scrollbar("refresh")}).bind("FilesAdded",
function(c,d){a.zluxdialog.toolbarBtnState(2,"upload","enabled");c.filelist.show();b.each(d,function(d,h){h.status="validating";var g={name:h.name,basename:h.name.replace(/(\.[a-z0-9]+)$/,""),type:"file",content_type:h.type,size:{size:h.size,display:plupload.formatSize(h.size)}};g.dom=b('<tr id="'+h.id+'" class="zlux-object" data-zlux-status="validating" />').append(b('<td class="column-icon" />').append('<i class="icon-file-alt zlux-x-object-icon" />'),b('<td class="column-name" />').append(a.renderObjectDOM(g))).appendTo(c.filelist);
b(".zlux-x-tools",g.dom).append(b('<span class="zlux-upload-file-progress"/>'));b(".zlux-x-name",g.dom).html(h.name);if(void 0!==h.size&&h.size>a.options.max_file_size){b(".icon-file-alt",g.dom).removeClass("icon-file-alt").addClass("icon-warning-sign");var f=a.pushMessageToObject(g,"File size error.<br />The file exceeds the maximum allowed size of "+plupload.formatSize(a.options.max_file_size));b(".zlux-x-msg-remove",f).remove();return!0}b.ajax({url:a.AjaxURL()+"&task=validateObjectName",type:"post",
data:{name:h.name},dataType:"json",cache:!1,beforeSend:function(a,c){b(".column-icon i",g.dom).addClass("icon-spinner icon-spin")},success:function(a){b(".zlux-x-name",g.dom).html(a.result);h.name=a.result;h.status=1;c._handleFileStatus(h);b(".column-icon i",g.dom).removeClass("icon-spinner icon-spin");c._updateFilelist()}})})}).bind("BeforeUpload",function(b){a.zluxdialog.toolbarBtnState(2,"cancel","enabled");a.zluxdialog.toolbarBtnState(2,"start","disabled");a.zluxdialog.toolbarBtnState(2,"add",
"disabled")}).bind("FileUploaded",function(c,d){b(".zlux-x-name",d.dom).html(d.name);b(".zlux-upload-file-progress",d.dom).html("100%").fadeOut();b(".zlux-x-object-icon",d.dom).removeClass("icon-file-alt").addClass("icon-ok");b(".zlux-x-name",d.dom).wrapInner('<a class="zlux-x-name-link" href="#" />').end().on("click","a",function(){var c=b('.zlux-object[data-id="'+d.name+'"]',a.filesmanager);d=a.oTable.fnGetData(c[0]);a.trigger("ObjectSelected",d);return!1})}).bind("FileNotUpload",function(c,d,e){a.zluxdialog.toolbarBtnState(2,
"cancel","disabled");a.zluxdialog.toolbarBtnState(2,"start","disabled");a.zluxdialog.toolbarBtnState(2,"add","enabled");b(".zlux-upload-file-progress",d.dom).fadeOut();b(".zlux-x-object-icon",d.dom).removeClass("icon-file-alt").addClass("icon-warning-sign");c=a.pushMessageToObject(d,e);b(".zlux-x-msg-remove",c).remove()}).bind("UploadComplete",function(b){a.zluxdialog.toolbarBtnState(2,"cancel","disabled");a.zluxdialog.toolbarBtnState(2,"start","disabled");a.zluxdialog.toolbarBtnState(2,"add","enabled");
a.reload()}).bind("CancelUpload",function(b){a.zluxdialog.toolbarBtnState(2,"cancel","disabled");a.zluxdialog.toolbarBtnState(2,"add","enabled")}).bind("QueueChanged",function(c,d){var e=!1;b.each(d,function(a,b){b.status!=plupload.DONE&&"validating"!=b.status&&(e=!0)});d.length?e?a.zluxdialog.toolbarBtnState(2,"start","enabled"):a.zluxdialog.toolbarBtnState(2,"start","disabled"):a.zluxdialog.toolbarBtnState(2,"start","disabled")}).bind("FileError",function(c,d,e){a.uploading&&"pending"==a.uploading.state()?
a.uploading.reject(e):(d.dom[0]||(d.dom=b('<tr id="'+d.id+'" class="zlux-object" />').append(b('<td class="column-icon" />').append('<i class="icon-file-alt zlux-x-object-icon" />'),b('<td class="column-name" />').append(a.renderObjectDOM(d))).appendTo(c.filelist),c._updateFilelist()),b(".zlux-x-object-icon",d.dom).removeClass("icon-file-alt").addClass("icon-warning-sign"),c=a.pushMessageToObject(d,e),b(".zlux-x-msg-remove",c).remove())})}});b.fn[f.prototype.name]=function(){var a=arguments,c=a[0]?
a[0]:null;return this.each(function(){var d=b(this);if(f.prototype[c]&&d.data(f.prototype.name)&&"initialize"!=c)d.data(f.prototype.name)[c].apply(d.data(f.prototype.name),Array.prototype.slice.call(a,1));else if(!c||b.isPlainObject(c)){var e=new f;f.prototype.initialize&&e.initialize.apply(e,b.merge([d],a));d.data(f.prototype.name,e)}else b.error("Method "+c+" does not exist on jQuery."+f.name)})}})(jQuery);
...
a.initDnDevents();a.bind("WindowDragHoverStart",function(b,d){a.dropzone.attr("data-zlux-draghover",!0)});a.bind("WindowDragHoverEnd",function(b,d){a.dropzone.attr("data-zlux-draghover",!1)});a.initFilelist();a.initPlupload();a.inited=!0},initFilelist:function(){var a=this;a.filelist=b('<table cellpadding="0" cellspacing="0" border="0" class="zlux-upload-filelist table table-striped table-bordered"><tbody /></table>').appendTo(a.upload).on("click",".zlux-x-remove",function(){if("uploding"!=b(this).closest(".zlux-object").data("zlux-status")){var c=
b(this).closest(".zlux-object"),d=a.uploader.getFile(c[0].id);d&&("undefined"==d||d.zlux_status!=plupload.STARTED&&d.status!=plupload.UPLOADING)?(a.uploader.removeFile(d),c.remove()):(c.remove(),a._updateFilelist())}})},initPlupload:function(){var a=this,c;c={runtimes:"html5, flash",browse_button:b(".zlux-upload-browse",a.upload)[0],drop_element:a.dropzone[0],max_file_size:void 0,url:a.AjaxURL()+"&task=upload",filters:[{title:"Files",extensions:this.options.extensions}],flash_swf_url:a.JRoot()+"plugins/system/zlframework/zlframework/zlux/assets/plupload/Moxie.swf"};
"s3"==a.options.storage&&(c=b.extend(c,{url:"http://"+a.options.storage_params.bucket+".s3.amazonaws.com",multipart:!0,multipart_params:{key:"${filename}",Filename:"${filename}",acl:"public-read",success_action_status:"201",AWSAccessKeyId:a.options.storage_params.accesskey,policy:a.options.storage_params.policy,signature:a.options.storage_params.signature},file_data_name:"file"}));c=b.extend(c,{init:{BeforeUpload:function(b,c){a.eventBeforeUpload(c)},UploadFile:function(b,c){a.eventUploadFile(c)},
UploadProgress:function(b,c){a.eventUploadProgress(c)},FileUploaded:function(b,c,h){a.eventFileUploaded(c,h)},UploadComplete:function(b,c){a.eventUploadComplete(c)},CancelUpload:function(b){a.eventCancelUpload()},FilesAdded:function(b,c){a.eventFilesAdded(c)},QueueChanged:function(b){a.eventQueueChanged()},StateChanged:function(){a.eventStateChanged()},Error:function(b,c){a.eventError(c)}}});a.uploader=new plupload.Uploader(c);a.uploader.bind("Init",function(){a.trigger("Init")});a.uploader.init()},
_:function(a){return plupload.translate(a)||a},eventError:function(a){var c=a.file,d,e;if(c){d="<strong>"+a.message+"</strong>";if(e=a.details)d+=" <br /><i>"+a.details+"</i>";else{switch(a.code){case plupload.FILE_EXTENSION_ERROR:e=this._("File: %s").replace("%s",c.name);break;case plupload.FILE_SIZE_ERROR:e=this.sprintf(this._("File: %s, size: %d, max file size: %d"),c.name,c.size,plupload.parseSize(this.options.max_file_size));break;case plupload.FILE_DUPLICATE_ERROR:e=this._("%s already present in the queue.").replace(/%s/,
c.name);break;case plupload.FILE_COUNT_ERROR:e=this._("Upload element accepts only %d file(s) at a time. Extra files were stripped.").replace("%d",this.options.max_file_count);break;case plupload.IMAGE_FORMAT_ERROR:e=this._("Image format either wrong or not supported.");break;case plupload.IMAGE_MEMORY_ERROR:e=this._("Runtime ran out of available memory.");break;case plupload.HTTP_ERROR:e="s3"==this.options.storage?this.options.storage_params.bucket.match(/\./g)?this._("The bucket name can't contain periods (.)."):
this._("There is some missconfiguration with the Bucket. Checkout the CORS permissions. If the bucket is recently created 24h must pass because of Amazon redirections."):this._("Upload URL might be wrong or doesn't exist.")}d+=" <br /><i>"+e+"</i>"}a={name:c.name,basename:c.name.replace(/(\.[a-z0-9]+)$/,""),type:"file",content_type:c.type,size:{size:c.size,display:plupload.formatSize(c.size)}};a.dom=b("#"+c.id,this.filelist);this.trigger("FileError",a,d)}},eventStateChanged:function(){this.uploader.state===
plupload.UPLOADING&&this.upload.attr("data-zlux-status","uploading");this.uploader.state===plupload.STOPPED&&(this.upload.attr("data-zlux-status","stopped"),this._updateFilelist())},eventBeforeUpload:function(a){var c=b("#"+a.id,this.filelist);"local"==this.options.storage&&(this.uploader.settings.url=this.uploader.settings.url+"&path="+this.options.path);"s3"==this.options.storage&&(this.uploader.settings.multipart_params.key=(this.options.path?this.options.path+"/":"")+a.name);b(".zlux-upload-file-progress",
c).html("0%");a.zlux_status=2;this._handleFileStatus(a);b(".zlux-upload-file-btn-remove",c).removeClass("icon-remove").addClass("icon-spinner icon-spin");this.trigger("BeforeUpload",a)},eventUploadFile:function(a){var c=this,d={name:a.name,basename:a.name.replace(/(\.[a-z0-9]+)$/,""),type:"file",content_type:a.type,size:{size:a.size,display:plupload.formatSize(a.size)}};d.dom=b("#"+a.id,c.filelist);c.uploading=b.Deferred().fail(function(a){c.trigger("FileNotUpload",d,a)}).done(function(a){d.name=
a;c.trigger("FileUploaded",d)}).always(function(b){c._handleFileStatus(a)})},eventUploadProgress:function(a){var c=isNaN(a.percent)?0:a.percent;b("#"+a.id+" .zlux-upload-file-progress",this.filelist).html(c+"%");this._handleFileStatus(a)},eventFileUploaded:function(a,c){b("#"+a.id,this.filelist);if("local"==this.options.storage){var d=b.parseJSON(c.response);d.error?this.uploading.reject(d.error.message):this.uploading.resolve(d.result)}else"s3"==this.options.storage&&(d=b(c.response),this.uploading.resolve(d.find("Key").html()))},
eventUploadComplete:function(a){this.trigger("UploadComplete",a)},eventCancelUpload:function(a){this.trigger("CancelUpload")},eventFilesAdded:function(a){this.trigger("FilesAdded",a)},eventQueueChanged:function(){this._updateFilelist()},getQueuedFiles:function(){var a=[];b.each(this.uploader.files,function(b,d){d.status!=plupload.DONE&&"validating"!=d.status&&a.push(d)});return a},emptyQueue:function(){this.uploader.splice();this.filelist.empty()},_updateFilelist:function(){var a=this,c=!1,d=b(".zlux-object",
a.filelist);b.each(a.uploader.files,function(b,d){d.status!=plupload.DONE&&"validating"!=d.status&&(c=!0);d.status==plupload.STOPPED&&a._handleFileStatus(d)});(a.uploader.files.length||d.length)&&b(".zlux-upload-dropzone-msg",a.upload).hide();a.uploader.files.length||d.length?c?a.upload.attr("data-zlux-status","queued"):a.upload.attr("data-zlux-status","stopped"):(a.upload.attr("data-zlux-status",""),b(".zlux-upload-dropzone-msg",a.upload).fadeIn());a.trigger("QueueChanged",a.uploader.files)},_handleFileStatus:function(a){var c=
b("#"+a.id,this.filelist),d="";a.zlux_status==plupload.STARTED?(d="started",a.zlux_status=""):(a.status==plupload.DONE&&(d="done"),a.status==plupload.FAILED&&(d="failed"),a.status==plupload.QUEUED&&(d="queued"),a.status==plupload.UPLOADING&&(d="uploading"),a.status==plupload.STOPPED&&b(".zlux-upload-file-progress",c).html(""));c.attr("data-zlux-status",d)},initDnDevents:function(a){var c=this,d=b(),e=b(),f=!1;inDZ=!1;b(window).on("drop",function(a){a.preventDefault();return!1}).on("dragenter",function(a){0===
d.size()&&(c.trigger("WindowDragHoverStart"),f=!0,inDZ=!1);d=d.add(a.target)}).on("dragleave drop",function(a){setTimeout(function(){var c=!1;try{c=b("body").find(a.relatedTarget).length?!0:c}catch(e){}d=d.not(a.target);0!==d.size()||c||(f=!1)},1);setTimeout(function(){f||inDZ||(c.trigger("WindowDragHoverEnd"),e=b(),d=b())},2)});c.dropzone.on("dragenter",function(a){0===e.size()&&(c.trigger("WindowDragHoverStart"),f=!1,inDZ=!0);e=e.add(a.target)});c.dropzone.on("dragleave drop",function(a){setTimeout(function(){var c=
!1;try{c=b("body").find(a.relatedTarget).length?!0:c}catch(d){}e=e.not(a.target);0!==e.size()||c||(inDZ=!1)},1);setTimeout(function(){f||inDZ||(c.trigger("WindowDragHoverEnd"),e=b(),d=b())},2)})}});b.fn[f.prototype.name]=f})(jQuery);
