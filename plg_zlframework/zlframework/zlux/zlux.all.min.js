...
(function(c){var f=function(a){this.options=c.extend({},this.options,a);this.events={}};f.prototype=c.extend(f.prototype,c.fn.zluxFilesManager.prototype,{name:"zluxDialogFilesManager",options:{title:"Files Manager",full_mode:0,dialogClass:""},events:{},initialize:function(a,b){this.options=c.extend({},c.fn.zluxFilesManager.prototype.options,this.options,b);var d=this;d.initCheck();d.dialogTrigger=c('<a title="'+d.options.title+'" class="btn btn-mini zlux-btn-edit" href="#"><i class="icon-edit"></i></a>').insertAfter(a).on("click",
...
browse_button:c('.zlux-dialog-toolbar .zlux-dialog-toolbar-2 i[data-id="add"]')});a.zluxupload.bind("QueueChanged",function(b){a.zluxdialog.scrollbar("refresh")}).bind("FilesAdded",function(b,d){a.zluxdialog.toolbarBtnState(2,"upload","enabled");b.filelist.show();c.each(d,function(d,h){h.status="validating";var g={name:h.name,basename:h.name.replace(/(\.[a-z0-9]+)$/,""),type:"file",content_type:h.type,size:{size:h.size,display:plupload.formatSize(h.size)}};g.dom=c('<tr id="'+h.id+'" class="zlux-object" data-zlux-status="validating" />').append(c('<td class="column-icon" />').append('<i class="icon-file-alt zlux-x-object-icon" />'),
c('<td class="column-name" />').append(a.renderObjectDOM(g))).appendTo(b.filelist);c(".zlux-x-tools",g.dom).append(c('<span class="zlux-upload-file-progress"/>'));c(".zlux-x-name",g.dom).html(h.name);if(void 0!==h.size&&h.size>a.options.max_file_size){c(".icon-file-alt",g.dom).removeClass("icon-file-alt").addClass("icon-warning-sign");var f=a.pushMessageToObject(g,a.sprintf(a._("FILE_SIZE_ERROR"),plupload.formatSize(a.options.max_file_size)));c(".zlux-x-msg-remove",f).remove();return!0}c.ajax({url:a.AjaxURL()+
"&task=validateObjectName",type:"post",data:{name:h.name},dataType:"json",cache:!1,beforeSend:function(a,b){c(".column-icon i",g.dom).addClass("icon-spinner icon-spin")},success:function(a){c(".zlux-x-name",g.dom).html(a.result);h.name=a.result;h.status=1;b._handleFileStatus(h);c(".column-icon i",g.dom).removeClass("icon-spinner icon-spin");b._updateFilelist()}})})}).bind("BeforeUpload",function(b){a.zluxdialog.toolbarBtnState(2,"cancel","enabled");a.zluxdialog.toolbarBtnState(2,"start","disabled");
...
c.fn[f.prototype.name]=function(){var a=arguments,b=a[0]?a[0]:null;return this.each(function(){var d=c(this);if(f.prototype[b]&&d.data(f.prototype.name)&&"initialize"!=b)d.data(f.prototype.name)[b].apply(d.data(f.prototype.name),Array.prototype.slice.call(a,1));else if(!b||c.isPlainObject(b)){var e=new f;f.prototype.initialize&&e.initialize.apply(e,c.merge([d],a));d.data(f.prototype.name,e)}else c.error("Method "+b+" does not exist on jQuery."+f.name)})}})(jQuery);
(function(c){var f=function(a){this.options=c.extend({},this.options,a);this.events={}};f.prototype=c.extend(f.prototype,c.fn.zluxMain.prototype,{name:"zluxUpload",options:{extensions:"",path:null,fileMode:"files",max_file_size:"1024kb",wrapper:null,storage:"local",storage_params:{},browse_button:null},init:function(){var a=this;a.upload=c('<div class="zlux-upload" />').attr("data-zlux-status","").appendTo(a.options.wrapper).hide();a.dropzone=c('<div class="zlux-upload-dropzone" />').appendTo(a.upload).append(c('<span class="zlux-upload-dropzone-msg" />'));
